...
OWL_GAME_ADDR=${owlGameAddress}
	...
	const receiver = "0xB2eC5D8Fd46089172dD394efe0fE5553f3fBB49e";
	for (i = 20; i < 170; i++) {
		console.log('transfer nft ', i);
		await owldinalNftContract.connect(deployer).mintByAdmin();
		await owldinalNftContract.connect(deployer).transferFrom(deployer.address, receiver, i);
	}
	// for (i = 20; i < 170; i++) {
	// 	await owldinalNftContract.connect(deployer).transferFrom(deployer.address, receiver, i);
	// }

	await owlTokenContract.connect(deployer).mint(receiver, BigInt(100000 * 100000) * decimal);
}
